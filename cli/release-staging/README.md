...
Run Codecane without the TUI for scripting and automation:

```bash
# Basic non-interactive usage
codecane -n "fix the bug in auth.ts"

# Pipe prompt from stdin
echo "explain this code" | codecane -n

# JSON output for structured parsing
codecane --json "analyze the codebase"

# Quiet mode (suppress streaming, show final result only)
codecane -q "run the tests"

# Set a timeout (in seconds)
codecane -n --timeout 60 "quick task"

# Write output to a file
codecane -n -o result.txt "generate a report"

# Combine with mode flags
codecane -n --max "complex refactoring task"
codecane -n --lite "simple question"
```
...
```bash
# List PRDs
codecane -n "/ralph list"

# Run orchestra mode
codecane -n "/ralph orchestra my-feature --parallelism 3"

# Get status as JSON
codecane --json "/ralph status my-feature"
```
